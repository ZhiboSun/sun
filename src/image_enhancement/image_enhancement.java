...
public class image_enhancement {
	public static void image_enhance(String pathname, String des1 , String des2){
		try {
            ...
            greyarray = Togrey.toGrey(srciImage);
            
            int newgray = 0;
            for (int i = 0; i < width; i++) {
				for (int j = 0; j < height; j++) {
					if(i!=0&&j!=0&&i!=width-1&&j!=height-1){
						newgray = prewitt(greyarray,i,j,width,height);
						result_image1.setRGB(i, j, newgray<<16|newgray<<8|newgray);	
						newgray = laplace(greyarray,i,j,width,height);
						result_image2.setRGB(i, j, newgray<<16|newgray<<8|newgray);
					}
